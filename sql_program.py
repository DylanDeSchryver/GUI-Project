...
def save_sql():
  ...
  crsr.execute("SELECT key FROM user WHERE key = {key}".format(key=key)) #generate key, make sure it does not exist already
  ans = crsr.fetchall() 
  while ans !=[]:
    key = random.randrange(100000,999999)
  ...
  crsr.execute(
      ...
      format(key=key, name=user_info['name'], last=user_info['last_name'], user=user_info['username'], passw=hashlib.sha256(user_info['password'].encode()).hexdigest())) #encrypt password using hashlib sha256 and base64 libraries
...
def get_sql(username,password):
  ...
  crsr.execute("SELECT key FROM user WHERE user = '{user}' AND pass = '{passw}'".format(user=username, passw = hashlib.sha256(password.encode()).hexdigest()))
...
def first(username,password):
  ...
  crsr.execute("SELECT fname FROM user WHERE user = '{user}' AND pass = '{passw}'".format(user=username, passw = hashlib.sha256(password.encode()).hexdigest()))
